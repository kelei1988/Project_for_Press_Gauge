...
long Average,Average0,Average1,Average2,pingjun,average_zd,test_num,last_test_num;//用于对ad的计算。
  ...
  keypad_Read_Setting_FormFlash(); //根据flash初始状态数值判断是第一次下载程序还是重启复位
    ...
    if(sh_dian!=0x3D)
    {
      ...
      { 
          ...
          { 
            pingjun =  Sample_value>>(ad_sample_count/32);
            Sample_count=0;
            Sample_value=0;
          }
          switch(zhuangtai)
...
void adjust_sample_count()
{
  ...
  while(key_value!=0x11)
  {
     ...
     if(sample_count_ram>5)
       sample_count_ram=5;
     if(sample_count_ram<1)
       sample_count_ram=1;
  }
  ad_sample_count=sample_count_ram*256;
}
...
__interrupt void ADC12ISR (void)
{
    ...
    {     
      if(Sample_count<ad_sample_count)
      {
       Sample_value= Sample_value+ADC12MEM0;
       Sample_count++;
      } 
      else
      {
        index5=ADC12MEM0;//Sample_value=0;
      }
      Sample_flag=1;  
    }
